...
# TODO: Get label for sample.
label = None
print('sample ' + str(i + 1) + ' is of class ' + label_dict[label, 1][0])
# TODO: Using the train_y data, compute the frequency of each label in
# a length 4 array
class_count = None
...
print('k-NN on pixel values: neighbor size %i maximises test accuracy' %
      best_k_test)
# TODO: Load, fit and predict on testing data with best_k_test. knn_best_predict
should
be
an
# array of predicted labels
knn_best_predict = None
# TODO: Finding wrongly classified samples for each class by comparing
knn_best_predict and train_y
for i in range(4):
...
# For each HSV converted image, compute the mean and standard deviation and make
new
8
feature
dataset
...
train_x_hsv_feature =
np.transpose(np.concatenate((train_x_hsv_mean, train_x_hsv_std), axis=0))
test_x_hsv_mean = np.mean(test_x_hsv, axis=(0, 1))
test_x_hsv_std = np.std(test_x_hsv, axis=(0, 1))
test_x_hsv_feature =
np.transpose(np.concatenate((test_x_hsv_mean, test_x_hsv_std), axis=0))
...
# TODO: Load, fit and predict on testing data with best_C_test.
svm_best_predicti_feature
should
be
an
# array of predicted labels
